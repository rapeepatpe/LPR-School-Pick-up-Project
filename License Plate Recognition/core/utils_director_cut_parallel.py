...
#             text2 = pytesseract.image_to_string(roi, lang = 'tha')
#             # print(pytesseract.tesseract_version())
...
def recognize_plate(img, coords):
    ...
    # im2_bitwise = cv2.bitwise_not(im2)
    # cv2.imshow("ex",im2_bitwise)
    # cv2.waitKey(0)
    
    plate_num = [None] * len(sorted_contours)
    plate_num_province = [None] * len(sorted_contours)
    
    def process_contour(cnt, index, plate_num, plate_num_province):
        x, y, w, h = cv2.boundingRect(cnt)
        height, width = im2.shape
        ratio = h / float(w)
        if 0.6 > ratio:
        ...
        try:
            if height / float(h) < 5.76:
                text = pytesseract.image_to_string(roi, config='--psm 10 --oem 3', lang="tha+eng")
                clean_text = re.sub('[\W_]+', '', text)
                plate_num[index] = clean_text
            else:
                text_province = pytesseract.image_to_string(roi, config='--psm 10 --oem 3', lang="tha")
                clean_text_province = re.sub('[\W_]+', '', text_province)
                plate_num_province[index] = clean_text_province
        except:
            text = None
            
    threads = []
    for i, cnt in enumerate(sorted_contours):
        t = threading.Thread(target=process_contour, args=(cnt, i, plate_num, plate_num_province))
        threads.append(t)
        t.start()
    ...
    if plate_num is not None and plate_num_province is not None:
        plate_num_str = str(plate_num)
        plate_num_province_str = str(plate_num_province)
        similar_ratio = []
        for i in province_list:
            similar_ratio.append(similar(plate_num_province_str,i))
        Accury_plate_num_province = province_list[similar_ratio.index(max(similar_ratio))]
        result_str = plate_num_str + plate_num_province_str
        print("License Plate #: ", result_str, Accury_plate_num_province)
    # cv2.imshow("Character's Segmented", im2)
    # cv2.waitKey(0)
    return result_str
######################   Original recognize_plate #########################
