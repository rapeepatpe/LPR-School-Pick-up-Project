...
def recognize_plate(img, coords):
    ...
    for cnt in sorted_contours:
        ...
        # print("height :",height)
        # print("h :",h)
        # print("relative height", height / float(h))
        # cv2.imshow("im2",im2)
        # cv2.waitKey(0)
        # if height of box is not tall enough relative to total height then skip
        # if height / float(h) > 8:
            # cv2.imshow("height case",im2)
            # cv2.waitKey(0)
            # print("height :",height)
            # print("h :",h)
            # continue
        ...
        try:
            if(height / float(h)<5.76):
                ...
                clean_text = re.sub('[\W_]+', '', text)
                plate_num += clean_text
                # cv2.imshow("roi_plate",roi)
                # cv2.waitKey(0)
            else:
                text_province = pytesseract.image_to_string(roi, config='--psm 10 --oem 3',lang = "tha")
                # clean tesseract text by removing any unwanted blank spaces
                # print("text_province = ",text_province)
                clean_text_province = re.sub('[\W_]+', '', text_province)
                plate_num_province += clean_text_province
                # cv2.imshow("roi_province",roi)
                # cv2.waitKey(0)
        except: 
            text = None
    if plate_num != None and plate_num_province != None:
        similar_ratio = []
        for i in province_list :
            # print(similar(plate_num_province,i))
            similar_ratio.append(similar(plate_num_province,i))
        # print(max(similar_ratio))
        # print(province_list[similar_ratio.index(max(similar_ratio))])
        # print(plate_num_province)
        Accury_plate_num_province = province_list[similar_ratio.index(max(similar_ratio))]
        print("License Plate #: ", plate_num, Accury_plate_num_province)
    # cv2.imshow("Character's Segmented", im2)
    # cv2.waitKey(0)
    return plate_num+plate_num_province
######################   Original recognize_plate #########################
